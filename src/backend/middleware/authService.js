...
export const registerUser = async (username, password) => {
  ...
  try {
    ...
    if (rows.length > 0) {
      authLogger.warn(
        `Registration attempt failed: User ${username} already exists.`
      );
...
export const loginUser = async (username, password) => {
  ...
  try {
    ...
    if (rows.length === 0) {
      authLogger.warn(
        `Login attempt failed: Invalid credentials for user ${username}.`
      );
    ...
    await connection.execute(
      "INSERT INTO access_tokens (username, token) VALUES (?, ?)",
...
export const refreshAccessToken = async (refreshToken) => {
  ...
  try {
    authLogger.debug(
      "This is in the refreshAccessToken function in authService.js"
    );
    ...
    await connection.execute(
      "UPDATE access_tokens SET token = ?, updated_at = CURRENT_TIMESTAMP WHERE username = ?",
...
export const logoutUser = async (username) => {
  const connection = await await pool.getConnection();
  try {
    await connection.execute("DELETE FROM access_tokens WHERE username = ?", [
      username,
    ]);
    await connection.execute("DELETE FROM refresh_tokens WHERE username = ?", [
      username,
    ]);
    authLogger.debug("This is in the logoutUser function in authService.js");
    authLogger.info(
      `User ${username} logged out successfully, tokens invalidated.`
    );
  } catch (error) {
